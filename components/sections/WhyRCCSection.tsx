  ...
  <svg width="120" height="140" viewBox="0 0 120 140" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden>
    ...
    {[0, 60, 120, 180, 240, 300].map((angle, i) => {
      ...
      return (
        <line
          key={i}
          x1={60 + 6 * Math.cos(rad)} y1={20 + 6 * Math.sin(rad)}
          x2={60 + 14 * Math.cos(rad)} y2={20 + 14 * Math.sin(rad)}
...
const GridSVG = () => (
  <svg width="140" height="120" viewBox="0 0 140 120" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden>
    {[0, 1, 2, 3, 4, 5, 6].map(i => (
      <line key={`v${i}`} x1={i * 23 + 1} y1="0" x2={i * 23 + 1} y2="120"
        stroke="#C9A84C" strokeWidth="0.8" strokeOpacity={0.12 + (i % 2) * 0.08} />
    ))}
    {[0, 1, 2, 3, 4, 5].map(i => (
      <line key={`h${i}`} x1="0" y1={i * 24 + 1} x2="140" y2={i * 24 + 1}
        stroke="#C9A84C" strokeWidth="0.8" strokeOpacity={0.12 + (i % 2) * 0.08} />
    ))}
    {[20, 47, 94, 117].map((cx, i) => [20, 68, 100].map((cy, j) => (
      <circle key={`${i}-${j}`} cx={cx} cy={cy} r="2.5"
        fill="#C9A84C" fillOpacity={0.25 + ((i + j) % 3) * 0.1} />
    )))}
...
export default function WhyRCCSection() {
  ...
  const row1In = useInView(row1Ref, { once: true, margin: '-5% 0px' });

  const row2Ref = useRef<HTMLDivElement>(null);
  const row2In = useInView(row2Ref, { once: true, margin: '-5% 0px' });

  const pillsRef = useRef<HTMLDivElement>(null);
  const pillsIn = useInView(pillsRef, { once: true, margin: '-5% 0px' });
  ...
  return (
    ...
    >
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', position: 'relative', zIndex: 1 }}>
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', opacity: 0.85, order: 0 }}>
            <GridSVG />
          </div>
          <div style={{ order: 1 }}>
            <div style={{
        ...
        >
          {PILLS.map((pill, i) => (
            <motion.div
              ...
              animate={pillsIn ? { opacity: 1, scale: 1 } : {}}
