    ...
    <section style={{ background: 'transparent', position: 'relative', padding: 'clamp(80px, 10vw, 140px) clamp(24px, 6vw, 120px)' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
          {STEPS.map((step, i) => (
            <div key={step.number} style={{ paddingBottom: i < STEPS.length - 1 ? 40 : 0, paddingLeft: 32 }}>
              <div
                style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(201,168,76,0.12)', borderRadius: 12, padding: 32, maxWidth: 480, transition: 'border-color 0.2s' }}
              ...
              >
                <div style={{ fontFamily: 'var(--font-dm-mono)', fontSize: 11, color: 'rgba(201,168,76,0.5)', letterSpacing: '0.12em', marginBottom: 10 }}>Step {step.number}</div>
                <div style={{ fontFamily: 'var(--font-montserrat)', fontSize: 20, fontWeight: 600, color: '#ffffff', marginBottom: 10 }}>{step.title}</div>
                <div style={{ fontFamily: 'var(--font-montserrat)', fontSize: 15, fontWeight: 300, color: 'rgba(255,255,255,0.45)', lineHeight: 1.6 }}>{step.desc}</div>
              </div>
            </div>
          ))}
        </div>
